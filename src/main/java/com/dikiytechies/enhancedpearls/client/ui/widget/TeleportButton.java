...
public class TeleportButton extends Button {
    ...
    public TeleportButton(int x, int y, int width, int height, PlayerEntity player, PlayerEntity target, IPressable event, Button.ITooltip tooltip) {
        super(x, y, width, height, ITextComponent.nullToEmpty(null), event, tooltip);
        this.player = player;
        this.target = target;
    }
    public TeleportButton(int x, int y, int width, int height, PlayerEntity player, PlayerEntity target, IPressable event) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float value) {
        ...
        if (this.visible) {
            renderPlayerFace(matrixStack, x + (int) (width / 4.5f), y + (int) (height / 4.5f), clientPlayer);
